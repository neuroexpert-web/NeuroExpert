    ...
    try {
      ...
      if (model === 'claude' && isValidAnthropicKey) {
        ...
        try {
          ...
          answer = `Привет! Я AI-помощник NeuroExpert v4.0 🚀

Вы спросили: "${question}"

🎯 Я готов помочь вам с:
• Анализом бизнес-процессов
• Стратегиями увеличения прибыли на 40%
• Автоматизацией рутинных задач
• Внедрением AI-решений

💡 Выберите интересующую вас тему или задайте конкретный вопрос!

${process.env.NODE_ENV !== 'production' ? `\n[Тестовый режим: API ключи не настроены]` : ''}`;
          usedModel = 'test-mode';
        }
      }
    } catch (error) {
